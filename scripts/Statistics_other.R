...
inlist = c('scc_IC0-IC0', 'scc_IC12-IC12', 'scc_IC27-IC27', 'scc_IC6-IC6')
...
OUTPUT = setNames(data.frame(matrix(ncol = 51, nrow = 0)), c("Folder", "Feature", "Type_number", "Slide_ROC.95.CI_lower", "Slide_ROC",                 
  ...
  tryCatch(
    {
      ...
      # Key names
      keydf = data.frame("Folder" = folder, "Feature" = pos, "Type_number" = type_number)
...
write.csv(New_OUTPUT, file = "~/documents/pancan_imaging/Results/Statistics_other.csv", row.names=FALSE)
