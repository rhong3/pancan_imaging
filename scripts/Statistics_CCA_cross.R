...
for (x in cancers){
  for(y in cancers){
    inlist = append(inlist, paste(feature, x, "on", y, "CCA", sep="_"))
  }
}

# Check previously calculated trials
previous=read.csv("~/Documents/pancan_imaging/Results/Statistics_CCA_cross.csv")
existed=paste(previous$Feature, previous$Model, "on", previous$On, "CCA", sep="_")
# Find the new trials to be calculated
targets = inlist[which(!inlist %in% existed)]
OUTPUT = setNames(data.frame(matrix(ncol = 51, nrow = 0)), c("Feature",	"Model",	"On", "Slide_ROC.95.CI_lower", "Slide_ROC",                 
...
for (i in targets){
  tryCatch(
    {
      pos = feature  #get positive case name
      print(i)
      folder = i  #split replicated trials
...
New_OUTPUT = New_OUTPUT[order(New_OUTPUT$Feature, New_OUTPUT$Model),]
write.csv(New_OUTPUT, file = "~/documents/pancan_imaging/Results/Statistics_CCA_cross.csv", row.names=FALSE)
